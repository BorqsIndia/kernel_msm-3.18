...
{
	if (strnstr(profile, ".cal", sizeof(".cal")) != NULL)
		return 1;
	return 0;
}
...
retry:
		...
		if (tfa98xx_kmsg_regs)
			dev_dbg(&tfa98xx->i2c->dev, "  WR reg=0x%02x,\
				val=0x%04x %s\n", subaddress, value,\
				ret < 0 ? "Error!!" : "");

		if (tfa98xx_ftrace_regs)
			tfa98xx_trace_printk("\tWR	 reg=0x%02x,\
					val=0x%04x %s\n", subaddress,\
					value, ret < 0 ? "Error!!" : "");
	} else {
...
retry:
		...
		if (ret < 0) {
			pr_warn("i2c error at subaddress 0x%x, retries left:\
				%d\n", subaddress, retries);
			if (retries) {
				retries--;
				msleep(I2C_RETRY_DELAY);
		...
		*val = value & 0xffff;
		...
		if (tfa98xx_kmsg_regs)
			dev_dbg(&tfa98xx->i2c->dev, "RD   reg=0x%02x,\
				val=0x%04x %s\n", subaddress, *val,\
				ret < 0 ? "Error!!" : "");
		if (tfa98xx_ftrace_regs)
			tfa98xx_trace_printk("\tRD	 reg=0x%02x,\
					val=0x%04x %s\n", subaddress, *val,\
					ret < 0 ? "Error!!" : "");
	} else {
		pr_err("No device available\n");
...
/* as Qualcomm Technologies, Inc. FAE suggested, we don't need call
'snd_pcm_hw_constraint_list' on their platform. */
