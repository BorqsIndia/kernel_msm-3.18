	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp("left", fs16xx->fs16xx_dev)) {
	id = FS16XX_DEV_INDEX_L;
	} else if (!strcmp("right", fs16xx->fs16xx_dev)) {
...
static void fs16xx_power(struct snd_soc_codec *codec, int on)
{
	...
	if (!strcmp("right", fs16xx->fs16xx_dev)) {
		  ...
		  char *buf, loff_t off, size_t count)
{
	...
	if (off + count > attr->size) {
		count = attr->size - off;
		PRINT_ERROR("trim count to %d", (int)count);
	}
	...
	if (fs16xx->codec && stereo_mode) {
		...
		if (count >= FS16XX_MAX_REGISTER * 2) {
			for (offset = 0; offset <= FS16XX_MAX_REGISTER;
				offset++) {
...
int fs16xx_i2c_probe(struct i2c_client *i2c,
			  const struct i2c_device_id *id)
{
	...
	fs16xx->vdd = regulator_get(&i2c->dev, "vdd");
...
EXPORT_SYMBOL(fs16xx_i2c_probe);
int fs16xx_i2c_remove(struct i2c_client *client)
{
	fs16xx_dev_t *fs16xx = i2c_get_clientdata(client);
...
EXPORT_SYMBOL(fs16xx_i2c_remove);
